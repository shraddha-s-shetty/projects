...
def exception_handler(target_function):
    def wrapper(*args, **kwargs):
        try:
            # Call the target function and return its result if no exception occurs
            return target_function(*args, **kwargs)
        except Exception as e:
            # Handle the exception here (or perform any other desired actions)
        ...
        if not self.title or not self.notes:
            ...
            for btn in button_list:
                try:
                    ...
                    print(e)
                    continue
            self.title = tk_window_dict[f"second_window_{curr_index}"].destroy()
